...
import { createPresence, ALLEGRO_URL, IPresence } from "@yomo/presence";
...
function Content({
  ...
  const presence = useRef<Promise<IPresence> | null>(null);
  if (presence.current === null) {
    presence.current = createPresence(ALLEGRO_URL, {
          ...
          >
            ...
            >
              ...
              <a
                ...
                href={"https://github.com/luzhenqian"}
              >
                {name}
              </a>
  ...
  useEffect(() => {
    if (exampleTheme === "") return;
    const _exampleTheme = examples[exampleTheme];
    if (_exampleTheme) {
      if (_exampleTheme.theme) {
        toggleColorScheme(_exampleTheme.theme as ColorScheme);
      }
      ...
      if (
        "avatarBackgroundColor" in _exampleTheme &&
        _exampleTheme.avatarBackgroundColor
      )
      ...
      >
        ...
        >
          ...
          <GroupHug
            ...
            MPOP={MPOP}
            // @ts-ignore
            onMouseEnter={() => eval(`(${onMouseEnterEditor?.getText()})()`)}
            // @ts-ignore
            onMouseLeave={() => eval(`(${onMouseLeaveEditor?.getText()})()`)}
